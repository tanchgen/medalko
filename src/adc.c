...
#define VAD_LIM_MIN   10000        // Минимальное напряжение накачки помпы
#define VAD_LIM_MAX   20000        // Максимальное напряжение накачки помпы
...
//            adcHandle.adcData[i].prm = ((adcHandle.adcData[ADC_PRM_VDD].prm * adcHandle.adcVprm[i]) / adcKprm[i]);
      ...
      case ADC_PRM_VAD:      // Напряжение накачки помпы
        ...
        else if( adcHandle.adcData[i].prm < adcHandle.adcData[i].prmLowAlrmLimit ){
